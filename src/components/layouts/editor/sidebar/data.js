...
class DataLoading extends Component {
    ...
    async componentDidMount() {
        ...
        if (projectName && data === defaultdata) {
            ...
            await axios.get(baseAPI.gitURL + "/repos/" + username + "/" + projectName + "/commits", { headers: { Authorization: 'token ' + accessToken } })
            .then(async (res) => {
                let fileTree;
                let lastCommit = res.data[0];
                let sha = lastCommit.sha;
                await axios.get(baseAPI.gitURL + "/repos/" + username + "/" + projectName + "/git/trees/" + sha + "?recursive=1", { headers: { Authorization: 'token ' + accessToken } }).then((res2) => {
                    fileTree = converttree.convertString(res2.data);
                }).catch();
                return fileTree;
    ...
    getContent(node) {
        if (node.type !== 'folder') {
            axios.get(node.content, {headers : {Authorization : 'token ' + localStorage.accessToken}}).then(gitResponse => {
                ...
                node.content = decodedData;
                node.oldcontent = decodedData;
            }).catch(err => {
                console.log(err);
            })
        }
